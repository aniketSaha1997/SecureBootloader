/**
  ******************************************************************************
  * @file    fwauth.c
  * @brief   File for fw signature check.
  *
  ******************************************************************************
  */
...
static int32_t Sha256HashDigestCompute(uint8_t* InputMessage, uint32_t InputMessageLength,
                                        uint8_t *MessageDigest, int32_t* MessageDigestLength);

static void FatalErrorHandler(void);
static void PrintBuffer(char *name, uint8_t * buf, uint32_t size);
...
extern int32_t FwVerify(void)
{
  ...
  printf("FW Meta data @0x%08lx:\r\n", FW_META_DATA_ADD);
#if defined STM32G474
  printf("FW Magic: 0x%08lx\r\n", pFWMeta->FWMagic);
#endif
  printf("FW Size: 0x%08lx\r\n", pFWMeta->FwSize);
  printf("FW Version: 0x%08lx\r\n", pFWMeta->FwVersion);
  PrintBuffer("FW HASH", pFWMeta->FwTag, FW_HASH_LEN);
  PrintBuffer("META DATA HASH", pFWMeta->MetaTag, FW_HASH_LEN);
  PrintBuffer("META DATA SIGNATURE", pFWMeta->MetaSig, FW_META_SIG_LEN);
  ...
  hash_status = Sha256HashDigestCompute((uint8_t*)pFWMeta, sizeof(FW_Meta_t)- FW_HASH_LEN - FW_META_SIG_LEN,
  ...
  if ((hash_status == HASH_SUCCESS) && (MetaDigestLength == FW_HASH_LEN))
  {
    ...
    int i;
    PrintBuffer("Computed META DATA HASH", MetaDigest, FW_HASH_LEN);
    ...
    PrintBuffer("META HASH", MetaDigest, MetaDigestLength);
  ...
  if (sig_status == SIGNATURE_VALID )
  {
    ...
    hash_status = Sha256HashDigestCompute((uint8_t*)FW_ADD, pFWMeta->FwSize,
                                        &MetaDigest[0], &MetaDigestLength);
    if ((hash_status == HASH_SUCCESS) && (MetaDigestLength == FW_HASH_LEN))
    {
      int i;
      PrintBuffer("FW HASH", MetaDigest, MetaDigestLength);
...
/**
  * @brief  Signature verification of firmware from metadata.
  * @param  pSignature: Pointer to firmware signature data.
  * @param  MessageDigest: Pointer to output parameter that will handle message digest
  * @param  MessageDigestLength: pointer to output digest length.
  * @retval error status
  *
...
/**
  * @brief  Custom print function to print the hash datas.
  * @param  name: Character string to display the name.
  * @param  buf : Data buffer to be printed.
