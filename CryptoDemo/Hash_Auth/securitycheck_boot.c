...
void CheckApplyStaticProtections(void)
{
  ...
  else
  {
    ...
    if (flash_option_bytes.WRPSector != BOOT_NB_PAGE_SEC_USER_MEM)
    {
			/* 
      ...
      printf("\r\nSecure User Memory Area size [0x%x bytes]\r\n", 
             flash_option_bytes.WRPSector * 0x00000800U);
      printf("\r\nExpected setting is [0x%x bytes]\r\n", BOOT_NB_PAGE_SEC_USER_MEM * 0x00000800U);
    }
    else
    {
      printf("\r\nSecure User Memory Area size [0x%x bytes]\r\n", flash_option_bytes.WRPSector * 0x00000800U);
